...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let path = read_link(format!("/proc/{}/fd/{}", pid, fd)).ok()?;

        let fdinfo_string = read_to_string(format!("/proc/{}/fdinfo/{}", pid, fd)).ok()?;
        let access_mode = Self::parse_access_mode(&fdinfo_string)?;
        let cursor = Self::parse_cursor(&fdinfo_string)?;
        let name = Self::path_to_name(path.to_str()?);

        let ans = Self::new(name, cursor, access_mode);
